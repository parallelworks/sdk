...
type AppResponse struct {
	...
	// The OIDC client_id, generated by the platform.
	ClientID string `json:"clientId"`
	// Whether the user sees a consent screen.
	...
	ID string `json:"id"`
	// The issuer tokens for this application are minted with.
...
type CreateWorkflowRunOutputBody struct {
	// Name of the session to open once the run starts.
	Redirect *string `json:"redirect,omitempty"`
	// Key into the run's links of the link to open once it resolves.
...
type ListAttachmentsBody struct {
	...
	HasMore bool `json:"hasMore"`
	// Pass as cursor to read the next page
	NextCursor *string `json:"nextCursor,omitempty"`
}
...
type ListUserAttachmentsBody struct {
	...
	// Pass as cursor to read the next page
	NextCursor *string `json:"nextCursor,omitempty"`
	// Matching attachments, capped at 10000
	Total int64 `json:"total"`
}
...
type PodLogsResponse struct {
	...
	// Set when the byte limit cut this page short. Poll again with lastSinceTime and lastLogHash to continue from where it stopped.
	Truncated *bool `json:"truncated,omitempty"`
}
...
type RealtimeAccuracyCategoryRow struct {
	// Cloud service provider
	Csp string `json:"csp"`
	// Sum of estimates
	Estimated float64 `json:"estimated"`
	// Sum of prorated invoice amounts
	Invoiced float64 `json:"invoiced"`
	// Deployment-day keys
	Keys int64 `json:"keys"`
	// Median per-key ratio over measurable keys
	MedianRatio *float64 `json:"medianRatio"`
	// Keys with an estimate but no invoice line
	NoInvoiceLine int64 `json:"noInvoiceLine"`
	// Estimated divided by invoiced; null without a measurable invoice
	Ratio *float64 `json:"ratio"`
	// Cost subtype
	Subtype string `json:"subtype"`
	// Cost type
	Type string `json:"type"`
	// Percentage of measurable keys whose ratio is within 0.9 to 1.1
	WithinTenPct *float64 `json:"withinTenPct"`
}

type RealtimeAccuracyOutlier struct {
	// Cloud service provider
	Csp string `json:"csp"`
	// Calendar day, YYYY-MM-DD
	Day string `json:"day"`
	// Deployment the estimate was attributed to
	DeploymentID string `json:"deploymentId"`
	// Estimated minus prorated invoice
	Error float64 `json:"error"`
	// Estimate for the key
	Estimated float64 `json:"estimated"`
	// Full-day invoice amount; null when no invoice line exists
	Invoiced *float64 `json:"invoiced"`
	// Invoice amount prorated to the estimated hours
	InvoicedProrated *float64 `json:"invoicedProrated"`
	// Estimated divided by prorated invoice; null without a measurable invoice
	Ratio *float64 `json:"ratio"`
	// The estimate came from a reprocess
	Reprocessed bool `json:"reprocessed"`
	// One of the resources behind the estimate
...
type RealtimeAccuracySummary struct {
	// How long the last successful rebuild took
	DurationMs *int64 `json:"durationMs"`
	// Error from the most recent failed rebuild, if any
	LastError *string `json:"lastError"`
	// When the accuracy data was last rebuilt; null before the first refresh
	RefreshedAt *time.Time `json:"refreshedAt"`
	// Whether a rebuild is currently running
	Refreshing bool `json:"refreshing"`
	// One row per CSP
	Summary []RealtimeAccuracySummaryRow `json:"summary"`
	Total   RealtimeAccuracySummaryRow   `json:"total"`
}

type RealtimeAccuracySummaryRow struct {
	// Cloud service provider
	Csp string `json:"csp"`
	// Sum of estimates
	Estimated float64 `json:"estimated"`
	// Sum of invoice amounts, prorated to the estimated hours
	Invoiced float64 `json:"invoiced"`
	// Deployment-day-category keys with a frozen estimate
	Keys int64 `json:"keys"`
	// Keys estimated by the running loop
	LiveKeys int64 `json:"liveKeys"`
	// Keys with an estimate but no invoice line
	NoInvoiceLine int64 `json:"noInvoiceLine"`
	// Estimated divided by invoiced; null without a measurable invoice
	Ratio *float64 `json:"ratio"`
	// Keys regenerated by a reprocess after the fact
	ReprocessedKeys int64 `json:"reprocessedKeys"`
	// Tagged invoice amount on keys the estimator produced no estimate for
	UncoveredInvoiced float64 `json:"uncoveredInvoiced"`
	// Keys with an invoice line and no estimate
	UncoveredKeys int64 `json:"uncoveredKeys"`
	// Weighted absolute percentage error over keys with an invoice line
	Wape *float64 `json:"wape"`
	// Percentage of measurable keys whose ratio is within 0.9 to 1.1
	WithinTenPct *float64 `json:"withinTenPct"`
}

type RealtimeAccuracyTrendPoint struct {
	// Cloud service provider
	Csp string `json:"csp"`
	// Calendar day, YYYY-MM-DD
	Day string `json:"day"`
	...
	// Sum of prorated invoice amounts
	Invoiced float64 `json:"invoiced"`
	// Keys contributing to the point
	Keys int64 `json:"keys"`
	// Estimated divided by invoiced; null without a measurable invoice
	Ratio *float64 `json:"ratio"`
	// Every key that day came from a reprocess
	Reprocessed bool `json:"reprocessed"`
}

type RealtimeAccuracyUncoveredRow struct {
	// Cloud service provider
	Csp string `json:"csp"`
	// Invoiced amount with no estimate behind it
	Invoiced float64 `json:"invoiced"`
	// Deployment-day keys with an invoice line and no estimate
	Keys int64 `json:"keys"`
	// Invoice lines behind those keys
...
type SecretInfo struct {
	...
	ExpiresAt *time.Time `json:"expiresAt,omitempty"`
	// Truncated preview of the secret for identification.
...
type SecretResponse struct {
	...
	// When a rotated secret stops working.
	ExpiresAt *time.Time `json:"expiresAt,omitempty"`
	// Truncated preview of the secret.
	Hint *string `json:"hint,omitempty"`
	// The identifier for this secret.
	ID string `json:"id"`
}
...
type Session struct {
	...
	WorkflowRunningSteps []RunStep `json:"workflowRunningSteps,omitempty"`
}
...
type WorkflowRunRedirect struct {
	Kind string `json:"kind"`
	Name string `json:"name"`
}
...
type WorkflowRunRunningStepsResponse struct {
	RunningSteps []RunStep `json:"runningSteps"`
}
